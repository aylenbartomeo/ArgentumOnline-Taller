...
public:
    TradeHandler(const ItemRegistry& registry, std::unordered_map<uint32_t, int>& stock, bool allowsSell) 
    ...
    bool execute(Player& player, const NpcCommandDTO& dto) override {
        ...
        if (dto.type == NpcCommandType::BUY) {
            ...
            if (!player.removeGold(static_cast<uint32_t>(unitPrice))) return true;
        ...
        if (dto.type == NpcCommandType::SELL) {
            if (!allowsSell) return true; // El Priest ignora este comando por completo
            ...
            if (playerSlot == -1) return true; // No lo tiene
