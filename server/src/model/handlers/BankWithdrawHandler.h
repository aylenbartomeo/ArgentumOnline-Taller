...
public:
    ...
    bool execute(Player& player, const NpcCommandDTO& dto) override {
        if (dto.type != NpcCommandType::WITHDRAW) return false;
        ...
        if (subComando == "oro") {
            uint32_t amount = 0;
            if (!(ss >> amount) || amount == 0) return true;

            if (!bankInstance.withdrawGold(playerId, amount)) return true;
        ...
        if (withdrawnAmount == 0) return true;
