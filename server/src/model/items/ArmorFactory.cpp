...
std::unique_ptr<Armor> ArmorFactory::create(const std::string& itemName) const {
    ...
    switch (itemConfig.slot) {
        case ArmorSlot::Body:
            return std::make_unique<BodyArmor>(itemConfig.id, itemName, itemConfig.price,
                                               itemConfig.minDefense, itemConfig.maxDefense);
        case ArmorSlot::Head:
            return std::make_unique<Helmet>(itemConfig.id, itemName, itemConfig.price,
                                            itemConfig.minDefense, itemConfig.maxDefense);
        case ArmorSlot::Shield:
            return std::make_unique<::Shield>(itemConfig.id, itemName, itemConfig.price,
                                              itemConfig.minDefense, itemConfig.maxDefense);
    }
