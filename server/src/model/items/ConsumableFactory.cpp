...
ConsumableFactory::ConsumableFactory(std::unordered_map<std::string, ConsumableConfig> configs):
...
std::unique_ptr<Consumable> ConsumableFactory::create(const std::string& itemName) const {
    ...
    return std::make_unique<Consumable>(cfg.id, itemName, cfg.price, cfg.type, cfg.durationMs,
                                        cfg.effectValue);
}
