...
bool BossZone::isBossAlive() const { return bossAlive; }
...
uint32_t BossZone::getBossEntityId() const { return bossEntityId; }

NPCType BossZone::getCurrentBossType() const { return currentBossType; }

Position BossZone::getSpawnPosition() const { return Position{config.spawnX, config.spawnY}; }

float BossZone::getRespawnCooldownMs() const { return config.respawnCooldownMs; }
