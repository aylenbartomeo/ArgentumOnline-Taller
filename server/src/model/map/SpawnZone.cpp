...
SpawnZone::SpawnZone(int x, int y, int width, int height, ZoneType type, 
                     const std::vector<NPCType>& allowedTypes, size_t maxMonsters, float cooldownMs)
    : x(x), y(y), width(width), height(height), zoneType(type), allowedMonsterTypes(allowedTypes),
      maxMonsters(maxMonsters), currentMonsterCount(0), 
      respawnCooldownMs(cooldownMs), timeSinceLastSpawnMs(cooldownMs) {
    // Inicializamos timeSinceLastSpawnMs en cooldownMs para que el primer tick pueda spawnear si hay espacio
}
...
std::optional<NPCType> SpawnZone::getRandomAllowedType() const {
    if (allowedMonsterTypes.empty()) return std::nullopt;
...
std::optional<Position> SpawnZone::getRandomSpawnPosition(int maxAttempts, const Map& map) const {
    ...
    if (maxX >= map.widthLimit()) maxX = map.widthLimit() - 1;
    if (maxY >= map.heightLimit()) maxY = map.heightLimit() - 1;
    ...
    for (int attempt = 0; attempt < maxAttempts; ++attempt) {
        ...
        if (zoneType == ZoneType::NORMAL) {
            ...
            bool inSpecificZone = false;
            for (const auto& zRect : map.getZoneRects()) {
                if (zRect.type != ZoneType::NORMAL && 
                    candidate.x >= zRect.x && candidate.x <= zRect.x + zRect.width &&
                    candidate.y >= zRect.y && candidate.y <= zRect.y + zRect.height) {
                    inSpecificZone = true;
                    break;
                }
            }
            if (inSpecificZone) continue;
        }
