...
std::vector<SpawnRequest> SpawnSystem::tick(float deltaTime, size_t currentMonsterCount,
                                            const Map& map, const EntityManager& entityManager) {
    std::vector<SpawnRequest> requests;
    if (monsterConfigs.empty())
        return requests;
...
std::optional<Position> SpawnSystem::findValidSpawnPosition(
        int maxAttempts, const Map& map, const EntityManager& entityManager) const {
    if (maxAttempts <= 0)
        return std::nullopt;
