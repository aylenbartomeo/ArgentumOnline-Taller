...
ZoneSpawnSystem::ZoneSpawnSystem(MonsterConfigs configs, float cooldownMs, size_t maxMon)
    : monsterConfigs(std::move(configs)), totalMaxMonsters(maxMon), respawnCooldownMs(cooldownMs) {
    
    // Clasificar monstruos por zona según config
    for (const auto& [type, config] : monsterConfigs) {
        if (config.isBoss) continue; // Bosses no van en zonas normales
...
void ZoneSpawnSystem::initializeZones(const Map& map) {
    ...
    for (const auto& rect : mapZones) {
        if (rect.type == ZoneType::NORMAL) continue; // La zona normal se crea al final
        ...
        zones.emplace_back(rect.x, rect.y, rect.width, rect.height, rect.type, allowed, 0, respawnCooldownMs);
    }
    ...
    zones.emplace_back(0, 0, map.widthLimit(), map.heightLimit(), ZoneType::NORMAL, normalAllowed, 0, respawnCooldownMs);
...
std::vector<SpawnRequest> ZoneSpawnSystem::getInitialSpawns(const Map& map) {
    ...
    if (zones.empty()) return requests;
...
void ZoneSpawnSystem::onMonsterDeath(const Position& pos) {
    ...
    for (auto& zone : zones) {
        if (zone.getType() != ZoneType::NORMAL && zone.contains(pos)) {
            zone.decrementCount();
            return;
        }
    }
