...
void CombatNotifier::notifyCombatResult(const Attackable& attacker, const Attackable& target,
    ...
    if (res.evaded) {
        ...
        if (pAttacker) {
            eventPublisher.sendTo(pAttacker->getDbId(),
                                  "¡" + target.getName() + " evadió tu ataque!");
        }
        ...
        if (pAttacker) {
            ...
            eventPublisher.sendTo(pAttacker->getDbId(),
                                  "¡Le hiciste " + std::to_string(res.damage) + " de dano a " +
                                          target.getName() + "!" + critMsg);
        }
