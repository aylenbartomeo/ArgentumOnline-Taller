...
private:
    ...
    std::optional<Position> findValidSpawnPosition(int maxAttempts, const Map& map, const EntityManager& entityManager) const;

public:
    explicit SpawnSystem(MonsterConfigs configs = {}, float cooldownMs = 5000.0f, size_t maxMon = 100);

    std::vector<SpawnRequest> tick(float deltaTime, size_t currentMonsterCount,
                                   const Map& map,
