...
uint16_t FormulaEngine::calculate_max_life(uint16_t constitution, float class_factor, 
    float race_factor, uint16_t level) {
...
uint32_t FormulaEngine::calculate_level_up_limit(uint16_t current_level) {
...
bool FormulaEngine::is_newbie(uint16_t level) {
    return level <= 12;
}

bool FormulaEngine::is_pvp_level_valid(uint16_t attacker_level, uint16_t victim_level) {
...
uint32_t FormulaEngine::calculate_safe_gold_limit(uint16_t level) {
    float result = 100.0f * std::pow(static_cast<float>(level), 1.1f);
    return static_cast<uint32_t>(result);
}

uint32_t FormulaEngine::calculate_excess_gold_dropped(uint32_t current_gold, uint32_t safe_limit) {
...
uint32_t FormulaEngine::calculate_npc_gold_drop(uint16_t npc_max_life) {
    RandomNumberGenerator rng;
...
uint16_t FormulaEngine::calculate_defense(uint16_t armor_min, uint16_t armor_max,
    ...
    uint16_t helmet_min, uint16_t helmet_max) {
    RandomNumberGenerator rng;
...
bool FormulaEngine::is_attack_eluded(uint16_t agility) {
    RandomNumberGenerator rng;
...
bool FormulaEngine::is_critical_attack(float critical_probability) {
    RandomNumberGenerator rng;
...
uint16_t FormulaEngine::calculate_passive_recovery(float race_recovery_factor, float seconds_elapsed) {
...
uint16_t FormulaEngine::calculate_meditation_recovery(float class_meditation_factor, 
                                            uint16_t intelligence, float seconds_elapsed) {
