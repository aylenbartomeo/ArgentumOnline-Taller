...
ClanOpResult ClanService::foundClan(uint32_t founderDbId, uint16_t founderLevel, const std::string& clanName) {
    if (repo.getClanIdOfPlayer(founderDbId)) return ClanOpResult::ALREADY_IN_CLAN;
    if (founderLevel < CLAN_MIN_LEVEL_TO_FOUND) return ClanOpResult::LEVEL_TOO_LOW;
    if (repo.isNameTaken(clanName)) return ClanOpResult::NAME_TAKEN;
...
ClanOpResult ClanService::joinRequest(uint32_t playerDbId, const std::string& clanName) {
    if (repo.getClanIdOfPlayer(playerDbId)) return ClanOpResult::ALREADY_IN_CLAN;
    
    Clan* clan = repo.getClanByName(clanName);
    if (!clan) return ClanOpResult::CLAN_NOT_FOUND;
    if (clan->isBanned(playerDbId)) return ClanOpResult::PLAYER_BANNED;
    if (clan->isFull()) return ClanOpResult::CLAN_FULL;
...
ClanOpResult ClanService::acceptMember(uint32_t founderDbId, uint32_t targetDbId) {
    ...
    if (clan->getFounderDbId() != founderDbId) return ClanOpResult::NOT_FOUNDER;
    if (!clan->hasPendingRequest(targetDbId)) return ClanOpResult::NO_PENDING_REQUEST;
    if (clan->isFull()) return ClanOpResult::CLAN_FULL;
...
ClanOpResult ClanService::kickMember(uint32_t founderDbId, uint32_t targetDbId) {
    ...
    if (!clan) return ClanOpResult::NOT_IN_CLAN;
    if (clan->getFounderDbId() != founderDbId) return ClanOpResult::NOT_FOUNDER;
    if (targetDbId == founderDbId) return ClanOpResult::NOT_FOUNDER; // No auto-kick
    if (!clan->isMember(targetDbId)) return ClanOpResult::PLAYER_NOT_FOUND;
...
ClanOpResult ClanService::leaveClan(uint32_t playerDbId) {
    Clan* clan = repo.getClanOfPlayer(playerDbId);
    if (!clan) return ClanOpResult::NOT_IN_CLAN;
    if (clan->getFounderDbId() == playerDbId) return ClanOpResult::CANNOT_LEAVE_AS_FOUNDER;
