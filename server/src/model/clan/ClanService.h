...
private:
    ClanRepository& repo;
    uint16_t minLevelToFound = 6;

public:
    explicit ClanService(ClanRepository& repo, uint16_t minLevel = CLAN_MIN_LEVEL_TO_FOUND):
            repo(repo), minLevelToFound(minLevel) {}

    void setMinLevelToFound(uint16_t level) { minLevelToFound = level; }

    ClanOpResult foundClan(uint32_t founderDbId, uint16_t founderLevel,
