...
void CharacterCreationScreen::drawText(const std::string& text, int x, int y, TTF_Font* useFont, const SDL_Color& color) {
...
void CharacterCreationScreen::drawStatRow(const std::string& label, int value, float raceFactor, int xLeft, int xRight, int y) {
    // Label a la izquierda
    drawText(label, xLeft, y, font, COLOR_WHITE);

    // Número a la derecha, con color según el factor
    SDL_Color color = COLOR_WHITE;
    if (raceFactor > 1.0f) color = COLOR_GREEN;
    else if (raceFactor < 1.0f) color = COLOR_RED;

    std::string numStr = std::to_string(value);
    
    // Calcular ancho para alinear a la derecha
    int w = 0, h = 0;
    if (font) TTF_SizeUTF8(font, numStr.c_str(), &w, &h);
    
    drawText(numStr, xRight - w, y, font, color);
}

void CharacterCreationScreen::drawSelector(const std::string& title, const std::string& value, int y) {
    int centerX = 585 + 112;

    // Título centrado
    int tw = 0, th = 0;
    if (fontLarge) TTF_SizeUTF8(fontLarge, title.c_str(), &tw, &th);
    drawText(title, centerX - tw/2, y, fontLarge, COLOR_WHITE);

    // Flechas fijas
    drawText("<", centerX - 100, y + 35, fontLarge, COLOR_WHITE);
    drawText(">", centerX + 80, y + 35, fontLarge, COLOR_WHITE);

    // Valor centrado entre las flechas
    int vw = 0, vh = 0;
    if (fontLarge) TTF_SizeUTF8(fontLarge, value.c_str(), &vw, &vh);
    drawText(value, centerX - vw/2, y + 35, fontLarge, COLOR_WHITE);
}

void CharacterCreationScreen::drawPreview(int x, int y) {
    std::string folder;
    switch(selectedRace) {
        case 0: folder = "resources/humano/"; break;
        case 1: folder = "resources/elfo/"; break;
        case 2: folder = "resources/enano/"; break;
        case 3: folder = "resources/gnomo/"; break;
    }

    try {
        SDL2pp::Texture& headTex = textures.get(folder + "cabeza-" + folder.substr(10, folder.length()-11) + ".png");
        SDL2pp::Texture& bodyTex = textures.get(folder + "cuerpo-" + folder.substr(10, folder.length()-11) + ".png");
        ...
        renderer.Copy(bodyTex, SDL2pp::Rect(0, 0, bw, bh), SDL2pp::Rect(x, y, bw * scale, bh * scale));

        int headX = x + (bw * scale - hw * scale) / 2;
        // Bajamos la base 1 pixel de escala (3 píxeles) para acercar las cabezas a los cuerpos
        int headY = y - (9 * scale);
        
        // Ajuste vertical para Enanos y Gnomos (acercamos un poco más si es necesario)
        if (selectedRace == 2 || selectedRace == 3) {
            headY += 13 * scale;
        }
        
        renderer.Copy(headTex, SDL2pp::Rect(0, 0, hw, hh), SDL2pp::Rect(headX, headY, hw * scale, hh * scale));
    } catch (...) {}
}
...
void CharacterCreationScreen::render() {
    ...
    drawSelector("Raza", RACE_NAMES[selectedRace], 195 + offY);
    ...
    int tw = 0, th = 0;
    if (fontLarge) TTF_SizeUTF8(fontLarge, "Atributos", &tw, &th);
    drawText("Atributos", 585 + offX - tw/2, attrY, fontLarge, COLOR_WHITE);
    
    attrY += 50;

    int raceOffset = selectedRace * 5;
    drawStatRow("Fuerza", currentStrength, configs.raceFactors[raceOffset + 2], xLeft, xRight, attrY);
    attrY += spacing;
    drawStatRow("Agilidad", currentAgility, configs.raceFactors[raceOffset + 3], xLeft, xRight, attrY);
    attrY += spacing;
    drawStatRow("Inteligencia", currentIntelligence, configs.raceFactors[raceOffset + 4], xLeft, xRight, attrY);
    attrY += spacing;
    
    drawStatRow("Vida Máxima", currentHp, configs.raceFactors[raceOffset], xLeft, xRight, attrY);
    attrY += spacing;
    
    drawStatRow("Maná Máxima", currentMana, configs.raceFactors[raceOffset + 1], xLeft, xRight, attrY);

    // Preview
    tw = 0; th = 0;
    if (fontLarge) TTF_SizeUTF8(fontLarge, "Aspecto", &tw, &th);
        ...
        while (SDL_PollEvent(&event)) {
            if (event.type == SDL_QUIT) {
                ...
                if (event.button.button == SDL_BUTTON_LEFT) {
                    ...
                    if (y >= BTN_CREAR_Y && y <= BTN_CREAR_Y + BTN_CREAR_H &&
                        x >= BTN_CREAR_X && x <= BTN_CREAR_X + BTN_CREAR_W) {
                        result.created = true;
